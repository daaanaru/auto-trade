...
6. ~~日経225全自動スクリーナー~~ → **完了**
7. ~~実弾投入レイヤー~~ → **完了**
8. ~~全市場統合スクリーナー~~ → **完了**（unified_screener.py）
9. ~~$200ペーパートレード開始~~ → **完了**（unified_paper_trade.py、5ポジション稼働中）
10. ~~自動卒業判定~~ → **完了**（graduation_checker.py）
11. ~~統合ダッシュボード~~ → **完了**（dashboard.py）
12. **$200ペーパートレード運用**（3/30まで毎日自動巡回）
13. **経過報告・戦略改善**（日次レポートを元に判断）
14. **卒業条件クリア → DRY_RUNテスト → 実弾投入判断**
15. ~~現金10%ガード~~ → **実装済み**（CASH_RESERVE_PCT = 0.10）
16. **市場時間フィルター未実装** → 閉場中のAPI呼び出しが無駄に走っている（本多智房に委任）
17. **BTC判断基準統一** → crypto_monitor(4戦略多数決) vs paper_trade(1戦略) の不一致を解消（鳳山担当）
