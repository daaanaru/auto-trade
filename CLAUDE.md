...
1. **ペーパートレード再スタート確認**（3/12夜に全面改修リセット実施済み。FX対応+市場時間フィルター追加。翌朝の再エントリー状況を確認）
2. **経過報告・戦略改善**（残り18日で卒業条件クリアを目指す）
3. **卒業条件クリア → DRY_RUNテスト → 実弾投入判断**
4. **~~市場時間フィルター未実装~~** → **3/12改修で対応済み**（market_hours.py追加）
5. **BTC判断基準統一** → crypto_monitor(4戦略多数決) vs paper_trade(1戦略) の不一致を解消（鳳山担当）
6. **BTC戦略の方針決定** → 上様に3択確認（卒業条件からBTC分離/戦略追加/放置）
